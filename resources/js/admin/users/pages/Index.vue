...
<script>
import FilterUser from './models/FilterUser.js'
...
export default {
  ...
  async created() {
    let filters = this.$localStorage.getItem(this.saveUserAlias)
    if (filters) {
  ...
  methods: {
    ...
    async showUserInfo(user) {
      let response = await this.$http.get(`${this.actions.rest}/${user.id}`)
    ...
    async edit(user) {
      await this.$router.push({name: 'update', params: {id: user.id}})
    },
    ...
    async remove(user) {
      this.errors = []
      try {
        await this.$http.delete('/api/v1/users/' + user.id)
        await this.getRecords()
      } catch (e) {
        if (!e.response) {
          throw e
